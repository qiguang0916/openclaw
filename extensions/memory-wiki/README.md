...
This plugin is separate from the active memory plugin. In deployments where
`mempalace-memory` owns `plugins.slots.memory`, MemPalace handles the active
recall and dreaming path while `memory-core` remains a legacy compatibility
lane. `memory-wiki` compiles durable knowledge into a navigable markdown vault
with deterministic indexes, provenance, and optional Obsidian CLI workflows.
...
- `isolated`: own vault, own sources, no dependency on the legacy `memory-core` lane
- `bridge`: reads public legacy `memory-core` artifacts and memory events through public seams
