...
// Download
const [download] = await Promise.all([page.waitForEvent("download"), page.click("a[download]")]);
...
await page.waitForFunction(() => document.querySelectorAll(".loaded").length >= 10);
```
